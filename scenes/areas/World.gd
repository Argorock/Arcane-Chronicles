...
func _enter_door(door):
	...
	print("ENTERING DOOR:", door.name, "target:", door.target_scene)
	
func _process(delta):
	if current_door and Input.is_action_just_pressed("ui_accept"):
		_enter_door(current_door)
	if current_door:
		print("Standing in:", current_door.name, "target:", current_door.target_scene)
...
func _on_arena_door_zone_body_entered(body):
	if body.name == "Player":
		current_door = $ArenaBuilding/ArenaDoorZone
...
func _on_arena_door_zone_body_exited(body):
	if body.name == "Player":
		current_door = null
